...
import CallType "./ICTC/CallType";
import Principal "mo:base/Principal";
import SyncTA "./ICTC/SyncTA";
...
shared(installMsg) actor class Example() = this {
    ...
    private func _taskCallback(_tid: SagaTM.Ttid, _task: SagaTM.Task, _result: SagaTM.TaskResult) : async (){
        taskLogs := Array.append(taskLogs, [(_tid, _task, _result)]);
    };
    private func _orderCallback(_oid: SagaTM.Toid, _status: SagaTM.OrderStatus, _data: ?Blob) : async (){
        orderLogs := Array.append(orderLogs, [(_oid, _status)]);
    };
    ...
    // ICTC: Query SagaTM logs
    public query func getTOCount() : async Nat{
        return _getSaga().count();
    };
    public query func getTO(_toid: SagaTM.Toid) : async ?SagaTM.Order{
        return _getSaga().getOrder(_toid);
    };
    public query func getTOs(_page: Nat, _size: Nat) : async {data: [(SagaTM.Toid, SagaTM.Order)]; totalPage: Nat; total: Nat}{
        return _getSaga().getOrders(_page, _size);
    };
    public query func getTOPool() : async [(SagaTM.Toid, ?SagaTM.Order)]{
        return _getSaga().getAliveOrders();
    };
    public query func getTT(_ttid: SagaTM.Ttid) : async ?SagaTM.TaskEvent{
        return _getSaga().getActuator().getTaskEvent(_ttid);
    };
    public query func getTTByTO(_toid: SagaTM.Toid) : async [SagaTM.TaskEvent]{
        return _getSaga().getTaskEvents(_toid);
    };
    public query func getTTs(_page: Nat, _size: Nat) : async {data: [(SagaTM.Ttid, SagaTM.TaskEvent)]; totalPage: Nat; total: Nat}{
        return _getSaga().getActuator().getTaskEvents(_page, _size);
    };
    
    public query func getTTPool() : async [(SagaTM.Ttid, SagaTM.Task)]{
    ...
    public query func getTTErrors(_page: Nat, _size: Nat) : async {data: [(Nat, SagaTM.ErrorLog)]; totalPage: Nat; total: Nat}{
        return _getSaga().getActuator().getErrorLogs(_page, _size);
    };
    public query func getCalleeStatus(_callee: Principal) : async ?SagaTM.CalleeStatus{
    ...
    public shared(msg) func removeTT(_toid: SagaTM.Toid, _ttid: SagaTM.Ttid) : async ?SagaTM.Ttid{
    ...
    public shared(msg) func appendTT(_toid: SagaTM.Toid, _callee: Principal, _callType: SagaTM.CallType, _preTtids: [SagaTM.Ttid]) : async SagaTM.Ttid{
        assert(_onlyBlocking(_toid));
        let saga = _getSaga();
        saga.open(_toid);
        let taskRequest = _buildTask(_callee, _callType, _preTtids);
        let ttid = saga.append(_toid, taskRequest, null);
        saga.finish(_toid);
        let f = saga.run(_toid);
        return ttid;
    };
    public shared(msg) func completeTO(_toid: SagaTM.Toid, _status: SagaTM.OrderStatus) : async Bool{
        assert(_onlyBlocking(_toid));
        _getSaga().finish(_toid);
    ...
    system func preupgrade() {
        __sagaData := Array.append(__sagaData, [_getSaga().getData()]);
    };
