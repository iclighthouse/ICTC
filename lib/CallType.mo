...
module {
    ...
    public func call(_args: CallType, _callee: Principal) : async TaskResult{
        var calleeId = Principal.toText(_callee);
        switch(_args){
            case(#DRC20(method)){
                ...
                switch(method){
                    case(#allowance(user, spender)){
                        ...
                        try{
                            // do
                            result := await token.allowance(user, spender);
                            // check & return
                    ...
                    case(#approve(spender, amount, nonce, sa, data)){
                        ...
                        try{
                            // do
                            result := await token.approve(spender, amount, nonce, sa, data);
                            // check & return
                            switch(result){
                    ...
                    case(#transfer(to, amount, nonce, sa, data)){
                        ...
                        try{
                            // do
                            result := await token.transfer(to, amount, nonce, sa, data);
                            // check & return
                            switch(result){
                    ...
                    case(#transferFrom(from, to, amount, nonce, sa, data)){
                        ...
                        try{
                            // do
                            result := await token.transferFrom(from, to, amount, nonce, sa, data);
                            // check & return
                            switch(result){
                    ...
                    case(#lockTransfer(to, amount, timeout, decider, nonce, sa, data)){
                        ...
                        try{
                            // do
                            result := await token.lockTransfer(to, amount, timeout, decider, nonce, sa, data);
                            // check & return
                            switch(result){
                    ...
                    case(#lockTransferFrom(from, to, amount, timeout, decider, nonce, sa, data)){
                        ...
                        try{
                            // do
                            result := await token.lockTransferFrom(from, to, amount, timeout, decider, nonce, sa, data);
                            // check & return
                            switch(result){
                    ...
                    case(#executeTransfer(txid, executeType, to, nonce, sa, data)){
                        ...
                        try{
                            // do
                            result := await token.executeTransfer(txid, executeType, to, nonce, sa, data);
                            // check & return
                            switch(result){
                    ...
                    case(#subscribe(callback, msgTypes, sa)){
                        ...
                        try{
                            // do
                            result := await token.subscribe(callback, msgTypes, sa);
                            // check & return
                    ...
                    case(#subscribed(user)){
                        ...
                        try{
                            // do
                            result := await token.subscribed(user);
                            // check & return
                    ...
                    case(#txnQuery(request)){
                        ...
                        try{
                            // do
                            result := await token.txnQuery(request);
                            // check & return
                    ...
                    case(#txnRecord(txid)){
                        ...
                        try{
                            // do
                            result := await token.txnRecord(txid);
                            // check & return
                    ...
                    case(#getCoinSeconds(user)){
                        ...
                        try{
                            // do
                            result := await token.getCoinSeconds(user);
                            // check & return
            ...
            case(_){ return (#Error, null, ?{code=#future(9901); message="No such canister."; });};
        };
    };
